...
final class OneVsRestModel(
  ...
  override def transformSchema(schema: StructType): StructType = {
    validateAndTransformSchema(schema, fitting = false, if (isDefined(classifier)) getClassifier.featuresDataType else new VectorUDT)
  }
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val probabilitiesUDF = udf { (predictions: Map[Int, Double]) =>
      predictions.toSeq.sortBy(_._1).map(_._2)
    }
    // output the index of the classifier with highest confidence as prediction
    val labelUDF = udf { (probs: Seq[Double]) =>
      probs.indexOf(probs.max).toDouble
    }
    // output label and label metadata as prediction
    val dataset2 = aggregatedDataset
      .withColumn(getProbabilityCol, probabilitiesUDF(col(accColName)))
      .withColumn(getPredictionCol, labelUDF(col(getProbabilityCol)), labelMetadata)
      .drop(accColName)
    if (!isDefined(probabilityCol)) {
      dataset2.drop(getProbabilityCol)
    } else {
      dataset2
    }
  }
...
/**
 *
 * MY VERSION OF OneVsRest THAT OUTPUTS THE PROBABILITY FOR EVERY CLASS, AND NOT JUST THE PREDICTION LIKE THE
 * ORIGINAL ONE SPARK.
 *
